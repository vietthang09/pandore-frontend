...
import { aiURL, baseURL, productSliderSettings } from "../../util/constants";
...
export default function ProductDetail() {
  ...
  async function loadRelatedProducts() {
    const response = await fetchAPI.sendGetRequest(
      `${aiURL}/recommender/${
        userRedux.user != null && userRedux.user != undefined
          ? userRedux.user.user_id
          : 0
      }`
    );
  ...
  const addToWishlistHandler = async () => {
    var requestData = {
      data: {
        ...
        productId: productInfor.product_id,
      },
    };
